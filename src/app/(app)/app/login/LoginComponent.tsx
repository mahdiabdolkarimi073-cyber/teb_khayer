...
const LoginComponent = (props: any) => {
	...
	return (
          <div>
               <p className="text-green-500 font-bold mb-3">قبلا ثبت نام کرده ام.</p>
...
export async function forgotPassword(phone: string) {
     ...
     modal("رمزعبور جدید", (
          <form action={async (form)=> {
               ...
               const r_pass = form.get('repeatPassword');
               if (pass !== r_pass) {
                    alert("تکرار رمزعبور اشتباه است")
               }

               const msg = await handleNewPassword(phone, pass+"");
               alert(msg);
               if (msg === "رمزعبور تغییر یافت") closeLastModal();
          }}>
               <PasswordInput
                    label={'رمزعبور جدید'}
                    name={'password'}
