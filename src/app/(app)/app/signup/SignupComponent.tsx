...
const SignupComponent = (props: { glass?: boolean }) => {
  return (
    <div className={props.glass ? styles.formCard : undefined}>
      <form action={async (formData: FormData) => {
        ...
        <TextInput label="نام و نام خانوادگی" required name="name" placeholder="نام..." />
        <NumberInput required label="شماره تلفن" name="phone" placeholder="09...." />
        <div className="center w-full gap-2 flex-col md:flex-row">
          <PasswordInput required className="flex-grow" label="رمزعبور" name="password" placeholder="****" />
          <PasswordInput required label="تکرار رمزعبور" className="flex-grow" name="repeat" placeholder="****" />
        </div>
    ...
    modal(
      (
        <div className="center gap-1">
          <p>کد تایید</p>
          <Button size="xs" variant="outline" onClick={send}>ارسال مجدد</Button>
        </div>
      ),
